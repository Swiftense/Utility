...
void shm_set(SHashmap *hm, K key, V value)
{
...
void shm_put(SHashmap *hm, K key, V value)
{
    /* resize map if there are more elements than buckets */
    if (hm->size++ == hm->capacity)
        shm_resize(hm);
...
int shm_contains(SHashmap *hm, K key)
{
...
void shm_remove(SHashmap *hm, K key)
{
    hasht index = shm_hash(key) % hm->capacity;
